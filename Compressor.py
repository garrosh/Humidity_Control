...
class Compressor(QObject):
  ...
  def counter_tick(self):
    ...
    if (not self.ready_state) & self.timer_counter > 18 & (not self.idle_state):
      self.timer_counter = 0
      self.set_ready()
    # Check if the counter is high enough to make the compressor idle
    if self.ready_state & self.timer_counter > 36 & (not self.idle_state):
      self.timer_counter = 0
      self.idle_state = True
      self.is_idle.emit()
    # Check if the counter is high enough to make it shutdown from overheating
    if self.compressor_state & self.timer_counter > 36:
      self.timer_counter = 0
